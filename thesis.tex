...
Now consider the same predicate, written in the CN specification language, as shown in Fig.~\ref{fig:intlist-cn}. The CN predicate definition is on the left hand side, with auxilliary C and CN definitions on the right. The CN definitions are enclosed in ``magic comments'' ({\color{commentcolour}$/*@ \ldots @*/$}), such that they can be parsed and desugared by the CN toolchain and treated like usual comments by any C compiler such as clang or gcc. 
