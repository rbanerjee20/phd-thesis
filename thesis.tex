...
\end{tikzpicture}
\caption{Fulminate architecture}\label{fig:fulm}
\end{figure}

architecture diagram (credit PS). CN+C file $\rightarrow$ Shared CN+Cerberus frontend $\rightarrow$ Typed Ail + Mucore $\rightarrow$ Ail to be injected back $\rightarrow$ Strings to be injected back $\rightarrow$ C file with instrumentation
