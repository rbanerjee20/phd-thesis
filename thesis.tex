...
Across the past several decades, a multitude of approaches have been put forward that aim to tackle this issue. These approaches vary across several axes, including: (1) their level of automation -- i.e.~whether they work on the software in question as-is, or require some further upfront effort\TODO{somehow express that latter can vary a lot -- e.g. inline assertions here and there vs complete specifications for verification. Maybe that comes in next para as I specialise to this case}; (2) the richness of the properties they can express and check; (3) their coverage of possible program paths, ranging from particular concrete executions to all arbitrary ones; (4) how integrable they are into conventional workflows -- whether incrementally, or at a particular stage of development, or not at all; and (5) how much they can be retrofitted to existing software. \TODO{Can 5 be subsumed by 4?} 

One end of this spectrum is made up of \emph{lightweight} methods\TODO{Does this word still make sense with AI?! Isn't specification and, as a result, verification, much cheaper and lighter now?}, which often involve some form of software testing -- checking certain properties\TODO{These properties are either implicit or explicit. Implicit property -- running program doesn't result in a runtime error} hold at runtime for a given program when exercised using some inputs.\footnote{`Testing' is often overloaded to mean either the runtime checks of the properties themselves, or the generation of the concrete inputs that drive these checks. Both of these are crucial to the overall quality of the testing.}
%
...
\begin{itemize}
  \item Reliability of large-scale software systems -- big problem since advent of modern computing 
  \item What kind of approaches are out there?
  \begin{itemize}
    \item Lightweight methods. Lightweight in terms of guarantees or upfront manual effort? Decide. I never know where to put static and dynamic analysis tools in this spectrum
  \end{itemize}
\end{itemize}
