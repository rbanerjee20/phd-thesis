...
In this chapter, we take a look at how Fulminate works at a high level, using a classic separation logic example of a singly-linked list, with the list and its operations implemented in C and its specifications written using CN. TODO: rewrite once chapter done
...
\section{A singly-linked list, in C}
...
Now consider the following \texttt{append} operation on two such singly-linked lists, also implemented in C:
...
To get higher assurance that this code behaves the way we expect it to, we might like to test it, or verify it, or both. We wish to describe that certain \emph{functional} properties hold (e.g.~what the integer elements of the returned, concatenated list are in relation to the elements of the input lists). In addition, since this is low-level C code that involves explicit pointers, we wish to describe \emph{ownership} properties as well -- here, we care especially about the structure of the lists in memory on entry to and exit from the function. For capturing both desired kinds of properties, separation logic comes to the rescue.



\section{The \texttt{IntList} predicate}

% \subsection{\texttt{IntList}, in classical separation logic}

To formally describe a linked list of the shape we just saw, consider the following separation-logic predicate, \texttt{IntList}:
...
This version of the \texttt{IntList} predicate \emph{looks} executable, and it turns out that it is. 
...
\section{Writing and testing specifications using Fulminate}
