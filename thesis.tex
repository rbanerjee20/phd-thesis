...
To get higher assurance that this code behaves the way we expect it to, we might like to test it, or verify it, or both. We wish to describe that certain \emph{functional} properties hold (e.g.~what the integer elements of the returned, concatenated list are in relation to the input lists); since this is low-level C code that involves explicit pointers, we wish to describe \emph{ownership} properties as well -- here, the structure of the lists in memory on entry to and exit from the function. For capturing both desired kinds of properties, separation logic comes to the rescue.
...
First, consider the following separation-logic predicate \texttt{IntList}:
