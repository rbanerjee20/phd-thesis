...
On one end of the spectrum, we have \emph{lightweight} methods, which usually involve some form of software testing (i.e.~checking properties over the concrete state at particular program points).\footnote{`Testing' is often overloaded to mean either the runtime checks of the properties themselves, or the generation of the concrete inputs that drive these checks. Here, we use `testing' to mean the former, and `test generation' to mean the latter; both of these are crucial to the overall quality of the testing.} In its simplest form, testing involves adding inline print and debug statements (e.g.~assertions) to a program, with some handwritten test case to drive the checks.  
%
