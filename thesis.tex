...
\section{What does it mean to test specifications?}
...
\section{Why test specifications?}
...
\section{[To delete] Initial bullet sketch}
...
\begin{enumerate}
  ...
  \item Related work (see first-year report, related work sections of papers)
    ...
    \end{enumerate}
\end{enumerate}
\chapter{Context: Testing separation-logic specifications}
...
\section{Executing separation logic}
...
\chapter{Fulminate: Motivating example}
...
\chapter{Code-Specify-Test-Debug-Prove, using Fulminate}

Working chapter title. Might be better not to put workflow name in title.


Include diagram from Keynote slides. Make diagram more generic; our work has been without the Code part of the cycle, so I made diagram specific to the retroactive case, but it needs to be more general. (Starts with ``Write partial specs'' which assumes a pre-existing codebase)
