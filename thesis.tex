...
To get higher assurance that this code behaves the way we expect it to, we might like to test it, or verify it, or both; this might naturally lead us towards wanting to write a specification for this function. Ideally, we would like our specification to:
...
\begin{itemize}
  ...
  \item Describe \emph{ownership} properties, since we are dealing with low-level C code that involves explicit pointers -- here, we care especially about the structure of the lists in memory on entry to and exit from the function.
  \item Be \emph{executable} for the purpose of testing, but also usable for verification if desired.
\end{itemize}

For the first two desiderata, separation logic comes to the rescue. The last one, however, is not obvious on first glance, for reasons we divulge in the next section.
...
something something, this allows us to start writing a specification. where does that discussion on executability go? :-(
