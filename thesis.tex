...
%
These approaches vary across several axes, including: (1) how automatable they are -- i.e.~whether they work on the software in question as-is\TODO{Implicit assumption of code-first? well, you need some software to check, right...}, or require some further upfront effort\TODO{somehow express that latter can vary a lot -- e.g. inline assertions here and there vs complete specifications for verification. Maybe that comes in next para as I specialise to this case}; (2) the richness of the properties they can express and check; (3) coverage of possible program paths, ranging from particular concrete executions to all arbitrary ones \TODO{This is just level of assurance, right?}; and (4) how integrable they are into conventional workflows -- whether incrementally, or at a particular stage of software development, or not at all.

Considering the first axis -- level of automation -- it is important to note that approaches on either end of the spectrum are not mutually exclusive; rather, they complement one another and have distinct benefits.\TODO{Maybe move to later in paragraph -- "it is clear" -- it will be clear after you've said the rest. It's not so clear a priori} More concretely, tools that are fully automatic (e.g.~static and dynamic program analysers, fuzzers\TODO{what's the implicit specification for a fuzzer? is there any beyond following function prototypes -- arg number and types? I guess not causing a compiler syntax error?}) rely on some notion of implicit specifications, ensuring properties such as memory safety and absence of undefined behaviour hold. These `push-button' tools are incredibly lightweight to use and eliminate entire classes of bugs, but the implicit nature of their specifications places an inherent limit on the richness of the properties they can check. The other end of this spectrum is formal verification, which involves writing explicit specifications -- both at the function level (preconditions and postconditions) and intermediate specifications needed for proof (e.g.~loop invariants and lemmas). Producing these specifications has historically been a complex, cumbersome task, with expert formal methods teams devoting years of effort towards these initiatives, all in exchange for high assurance: strong correctness guarantees over all arbitrary executions. And beyond the extrema, there are various approaches in between, all of which involve the same tradeoff between upfront\TODO{I am trying to be careful in not using the word manual, in the age of AI} effort and correctness guarantees. This includes several kinds of software \emph{testing}: e.g.~simple inline print and debug statements (e.g.~assertions), fuzzing, property-based testing and symbolic execution. 

TODO: Other end of the spectrum is full formal verification with explicit specifications. In the middle is testing (inline assertions, contract checking, \emph{specification-testing}, property-based testing --  but not fuzzing, we mention that earlier with program analysis tools)

-----


One end of this spectrum is made up of \emph{lightweight} methods, which often involve some form of software testing -- checking certain properties\TODO{These properties are either implicit or explicit. Implicit property -- running program doesn't result in a runtime error} hold at runtime for a given program when exercised using some inputs.\footnote{`Testing' is often overloaded to mean either the runtime checks of the properties themselves, or the generation of the concrete inputs that drive these checks. Both of these are crucial to the overall quality of the testing.}
%
The simplest example of this is unit testing -- manually-provided inputs for driving some function, often to exercise handwritten runtime assertions at particular program points --  
% In its simplest form, testing involves adding inline print and debug statements (e.g.~assertions) to a program, with some handwritten test case to drive the checks. More sophisticated testing techniques involve strengthening the properties being runtime-checked, and/or the quality of the inputs used to drive the checks. The latter is a well-explored area, encompassing techniques such as fuzzing, property-based testing and symbolic execution, with some tradeoff between performance and how tailored the inputs are to the system under test.

% (ranging from `dumber' black-box fuzzing, which quickly generates more randomised inputs to `smarter' white-box fuzzing, where constraints over the structure of the input are known in advance);
symbolic execution and property-based testing,   Typically, the stronger the checks, the higher the cost of the testing, either in performance or in manual effort. 
%
